...
import com.vcare.api.Gateway.EmailModel;
import com.vcare.api.Gateway.IEmailModel;
...
//            		"testSms5";
        if (isAsync) { // Send asynchronously
            p.send(new ProducerRecord<>(topic,
                    messageNo + "",
                    sms), new com.vcare.api.kafka.DemoCallBack(startTime, messageNo, messageStr));
        } else { // Send synchronously
            try {
                p.send(new ProducerRecord<>(topic,
                        messageNo + "",
